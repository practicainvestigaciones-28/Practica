...
function Convocatorias() {
  ...
  const [periodos, setPeriodos] = useState<Periodo[]>([
    { id: 1, nombre: '2026-1', activo: true },
    { id: 2, nombre: '2026-2', activo: false }
  ])
  ...
  const [periodoNombreForm, setPeriodoNombreForm] = useState<string>('')
  ...
  const handleRegistrarPeriodo = () => {
    ...
    if (periodoModoFormulario === 'editar' && periodoEditandoId !== null) {
      setPeriodos(periodos.map(p => p.id === periodoEditandoId ? { ...p, nombre: periodoNombreForm.trim() } : p))
    } else {
      setPeriodos([...periodos, { id: Date.now(), nombre: periodoNombreForm.trim(), activo: true }])
    }
    setPeriodoModal('exito')
  }

  const handleTogglePeriodo = (id: number) => {
    setPeriodos(periodos.map(p => p.id === id ? { ...p, activo: !p.activo } : p))
  }
  ...
  return (
    <div className="conv-page">
      {!modoFormulario && !periodoModoFormulario ? (
        <>
          ...
          <div className="periodo-modal-wrapper">
            <div className="periodo-modal-box">
              ...
              <h2 className="periodo-modal-title">
                {periodoModoFormulario === 'editar' ? 'Editar período' : 'Registrar período de duración'}
              </h2>
...
export default Convocatoriasimport { useState, useEffect } from 'react'
import { FilePlus, Search, SquarePen, Trash2, Save, X as XIcon } from 'lucide-react'
import DateRangeCalendar, { CalendarIcon, formatearRango } from '../components/DateRangeCalendar'
import ConfirmModal from '../components/ConfirmModal'
import * as convocatoriasApi from '../api/convocatorias'
